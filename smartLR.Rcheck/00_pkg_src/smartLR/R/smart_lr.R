...
#' @export
smart_lr <- function(formula, data, weight = replicate(nrow(data), 1)) {
  ...
  ifelse((arma_method == TRUE), fastLmX(formula, data, weight), lm(fastLmX(formula, data, weights = weight)))
 }
...
#' @export
smart_Pure <- function(X, Y, Z = replicate(nrow(data), 1)) {
